...
<tbody>
	...
	<tr>
		...
		<td>
		<select name="assigned_to" class="inputbox">
		<option value="0"><?php echo JText::_('No-one');?></option>
		<?php
		foreach($users as $vkey=>$vvalue)
		{
			$selected = ($bug->assigned_to == $vkey)?'Selected':'';
			echo '<option value="'.$vkey.'" '.$selected.'>'.$vvalue.'</option>';
		}
		?></select>
		</td>
	...
	<tr>
		<td nowrap>
		 <?php echo JText::_('Bug Status: ');?>
		</td>
		<td>
		<select name="bug_status_id"  class="inputbox">
		<?php 
		foreach($options['status'] as $pkey=>$pvalue)
		{
			$selected = ($pkey == $bug->bug_status_id)?'Selected':'';
			echo '<option value="'.$pkey.'" '.$selected.'>'.$pvalue.'</option>';
		}?>
		</select>
		</td>
